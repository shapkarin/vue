...
It use `@starport/vue` and `@starport/vuex` from [tendermint/vue monorepo](https://github.com/tendermint/vue).
...
```
It's defined at the `.env` file.
```

<Preview>
  <Story name="Welcome/CheckEnviroment" parameters={{
      viewMode: 'docs',
  }}>
    {{
      setup() {
      ...
      template: `<div style="fontSize: 24px">
        <div>apiTendermint: {{apiTendermint}}</div>
        <div>apiCosmos: {{ apiCosmos }}</div>
        <div>apiWS: {{ apiWS }}</div>
      </div>`,
    }}
  </Story>
